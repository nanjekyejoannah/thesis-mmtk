...
class MarkSweepGC(GCBase):
    ...
    def __init__(self, config, chunk_size=DEFAULT_CHUNK_SIZE, start_heap_size=4096):
        ...
        ll_mmtk_pypy_gc_init(lltype.nullptr(self.HDR), self.param_start_heap_size)
    ...
    def malloc_fixedsize_clear(self, typeid16, size, can_collect,
        ...
        try:
            ...
            raise memoryError
        result = llmemory.cast_adr_to_int (ll_mmtk_alloc(tot_size, tot_size, size, size_gc_header, 0))
        if not result:
            raise memoryError
    ...
    def malloc_varsize(self, typeid16, length, size, itemsize,
        ...
        try:
            ...
            raise memoryError
        result = result = llmemory.cast_adr_to_int (ll_mmtk_alloc(tot_size, tot_size, size, size_gc_header, 0))
        if not result:
            raise memoryError
    ...
    def collect(self, gen=0):
        ...
        while poolnode:   #sweep
            ...
            while hdr:  #sweep
                mmtk_initialize_collection(self.vm_thread_mutator)
        ...
        debug_print("  malloced since previous collection:",
                    ll_mmtk_total_bytes(), "bytes")
        debug_print("  heap usage at start of collection: ",
                    ll_mmtk_used_bytes(), "bytes")
        debug_print("  freed:                             ",
                    ll_mmtk_free_bytes(), "bytes")
