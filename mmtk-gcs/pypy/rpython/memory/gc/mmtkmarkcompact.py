...
class MMTKMarkCompactGC(MovingGCBase):
    ...
    def malloc_fixedsize_clear(self, typeid16, size, is_finalizer_light=False,
    ...
    def update_run_finalizers(self):
        ...
        if self.objects_with_light_finalizers.non_empty():     # uncommon case
            objects_with_light_finalizers = self.AddressDeque()
            while self.objects_with_light_finalizers.non_empty():
                obj = self.objects_with_light_finalizers.pop()
                objects_with_light_finalizers.append(self.get_forwarding_address(obj))
    ...
    def trace_from_objects_with_finalizers(self):
        if self.objects_with_finalizers.non_empty():   # uncommon case
            ...
            self.objects_with_finalizers.delete()
            self.objects_with_finalizers = new_run_finalizers
        #
        ...
        while self.objects_with_finalizers.non_empty():
            ...
            if self.marked(x):
                ...
                self.objects_with_finalizers.append(x)
                self.mark(x)
                self._trace_and_mark()
        self.objects_with_finalizers.delete()
